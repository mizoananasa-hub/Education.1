...
export default function TeacherSignup() {
  ...
  const signupMutation = useMutation({
    mutationFn: async (data: FormData) => {
      const res = await fetch(getApiUrl("/api/auth/teacher/signup"), {
        ...
        <div className="w-full max-w-md text-center">
          <div className="bg-card border border-card-border rounded-2xl shadow-xl p-12">
            ...
            <p className="text-muted-foreground mb-6">
              Your teacher account request has been submitted. An administrator will review it and assign your subjects. You'll be able to sign in once approved.
            </p>
      ...
      <div className="w-full max-w-lg">
        ...
        <div className="bg-card border border-card-border rounded-2xl shadow-xl overflow-hidden">
          <div className="p-8 sm:p-12">
            ...
            <h1 className="text-3xl font-bold tracking-tight mb-2">Teacher Account</h1>
            <p className="text-muted-foreground mb-2">Apply for a teacher account on Learnova.</p>
            <div className="flex items-center gap-2 text-xs text-amber-600 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2 mb-8">
              ...
              Your account requires admin approval. Subjects will be assigned by the administrator.
            </div>
            <Form {...form}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <FormField control={form.control} name="password" render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl><Input type="password" placeholder="Min. 6 characters" {...field} className="h-12" /></FormControl>
                      <FormMessage />
                    </FormItem>
                  )} />
                  <FormField control={form.control} name="confirmPassword" render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl><Input type="password" placeholder="Repeat password" {...field} className="h-12" /></FormControl>
                      <FormMessage />
                    </FormItem>
                  )} />
                </div>
                <Button type="submit" className="w-full h-12 text-base font-medium" disabled={signupMutation.isPending}>
                  {signupMutation.isPending ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" /> Submitting…</> : "Submit Application"}
                </Button>
              </form>
            </Form>
