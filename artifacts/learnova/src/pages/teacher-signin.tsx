...
export default function TeacherSignin() {
  ...
  const signinMutation = useMutation({
    mutationFn: async (data: FormData) => {
      const res = await fetch(getApiUrl("/api/auth/teacher/signin"), {
    ...
    onSuccess: (res) => {
      login(res.token);
      toast({ title: "Welcome back!", description: "Signed in to Teacher Portal." });
      setLocation("/teacher");
    },
        ...
        <div className="bg-card border border-card-border rounded-2xl shadow-xl overflow-hidden">
          <div className="p-8 sm:p-12">
            ...
            <div className="mt-8 text-center text-sm text-muted-foreground border-t border-border pt-6">
              ...
              <Link href="/teacher/signup" className="text-primary font-medium hover:underline">Create one</Link>
            </div>
          </div>
        </div>
      </div>
